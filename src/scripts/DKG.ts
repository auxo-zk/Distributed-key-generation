...
async function main() {
  ...
  const fetchAllContract = async (contracts: {
    ...
    while (attempts < maxAttempts) {
      try {
        const fetchPromises = Object.entries(contracts).map(
          ([key, { contract }]) => {
            const fetchActionsPromise = Mina.fetchActions(contract.address);
            const fetchAccountPromise = fetchAccount({
              publicKey: contract.address,
            });
            return Promise.all([fetchActionsPromise, fetchAccountPromise]);
          }
        );
  ...
  let memberNonces: number[] = [];

  for (let i = 0; i < members.length; i++) {
    let sender = await fetchAccount({ publicKey: members[i].publicKey });
    memberNonces.push(Number(sender.account?.nonce) - 1);
  }
