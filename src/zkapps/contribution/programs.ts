...
import { calculateKeyIndex, KeyStorage } from '../key/index.js';
import { EncryptionStorage, PolynomialCommitmentStorage } from './storages.js';
import { calculateShareCommitment } from '../../libs/Committee.js';
...
/**
 * Note: Scaling Group points with Field values here is safe if max{T, N} <= 45
 * N ^ (T-1) <= Field.ORDER - 1
 */
        ...
        decrypt: {
            ...
            async method(input: BatchDecryptionInput, privateKey: PrivateKey) {
                ...
                for (let i = 0; i < INST_LIMITS.MEMBER; i++) {
                    ...
                    share = new ScalarField(
                        share
                            .add(ScalarField.fromScalar(Scalar.fromField(m)))
                            .assertCanonical()
                    );
                }
                input.commitment.assertEquals(
                    calculateShareCommitment(
                        ScalarField.toScalar(share),
                        input.memberId
                    ),
                    Utils.buildAssertMessage(
                        BatchDecryption.name,
